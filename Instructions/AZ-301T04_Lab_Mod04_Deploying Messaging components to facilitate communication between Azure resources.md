...
1. **名前空間を作成** ブレードで、次のタスクを実行します。
    ...
    - **価格レベル** ドロップダウン リストで、**基本(Basic)** オプションを選択します。 
...
1. **ストレージ アカウントの作成** ブレードで、次のタスクを実行します。
    ...
    - 「**ネットワーク**」タブの構成オプションは、既定値に設定したままにします。 

    - 「**詳細**」タブの **安全な転送が必要** セクションで、 **無効** オプションが選択されていることを確認します。  
...
1. 「ストレージ アカウント」 ブレードで、 **コンテナ** タイルをクリックします。 
...
1. **新規コンテナー** ペインで、次のタスクを実行します。 
    ...
    - **パブリック アクセスレベル**  ドロップダウン リストで、**Blob (BLOB 専用の匿名読み取りアクセス)** オプションを選択します。
...
1. **ロジック アプリ デザイナー** ブレードで、下にスクロールし、**テンプレート** セクションの **「空白のロジック アプリ」** タイルをクリックします。     

1. **ロジック アプリ デザイナー** ブレードで、次のタスクを実行します。 

    - **検索コネクタとトリガー** テキスト ボックスで、  **Service Bus** と入力します。

    - 検索結果で、 **キューで1メッセージをつ以上のメッセージを受信したとき(オートコンプリート)** という名前のトリガーを選択します。 

    - **接続名** テキスト ボックスに、 **ServiceBusConnection** と入力します。
    
    - **Service Bus 名前空間** の一覧で、この実習ラボで前に作成した名前空間を選択します。 

    - ポリシーの一覧で、 **RootManageSharedAccessKey(ルート管理共有アクセスキー)** ポリシーを選択します。 

    - **作成** ボタンをクリックします。

1. **キューメッセージを受信した場合(自動完了)** のステップでは、次のタスクを実行します。

    - **キュー名** ドロップダウン リストで、 **messages** エントリを選択します。 

    - **項目を確認する頻度** テキスト ボックスで、**30** と入力します。

    - **頻度** ドロップダウン リストで、 **秒** を選択します。
...
```
    > **注記** 出力には **undefined** が表示されます。これは予測されていることです。。
...
1. 「ストレージ アカウント コンテナー」 ブレードで、 **messageoutput** コンテナーをクリックします。 
